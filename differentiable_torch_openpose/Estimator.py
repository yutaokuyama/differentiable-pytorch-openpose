...
class Estimator(object):
    ...
    def __call__(self, oriImg: torch.Tensor):
        ...
        stride = 8
        padValue = 128
        multiplier = scale_search * boxsize / oriImg.shape[0]

        scale = multiplier
        imageToTest = F.resize(oriImg.permute(2, 0, 1), (int(
            oriImg.shape[0]*scale), int(oriImg.shape[1]*scale))).permute(1, 2, 0)

        imageToTest_padded, pad = util.padRightDownCorner(
            imageToTest, stride, padValue)

        im = imageToTest_padded.unsqueeze(3).permute(3, 2, 0, 1) / 256 - 0.5

        data = (im.contiguous()).cuda()

        _, Mconv7_stage6_L2 = self.model(data)

        heatmap = Mconv7_stage6_L2.squeeze(0)  # output 1 is heatmaps

        heatmap = F.resize(heatmap, (int(
            heatmap.shape[1]*stride), int(heatmap.shape[2]*stride))).permute(1, 2, 0)
        ...
        heatmap = F.resize(
            heatmap, (oriImg.shape[1], oriImg.shape[0])).permute(2, 1, 0)

        return heatmap

    def detect_keypoints(self, oriImg):
        ...
        for m in range(len(multiplier)):
            ...
            imageToTest_padded, pad = util.padRightDownCorner_np(
                imageToTest, stride, padValue)
    ...
    def debug_heatmap(self, heatmap: torch.Tensor, oriImg: torch.Tensor):
        heatmap_np = heatmap.detach().cpu().numpy().copy()
        print("heatmap_np", heatmap_np.shape)
        oriImg_np = oriImg.detach().cpu().int().numpy().copy()
        print("oriImg_np", oriImg_np.shape)

        for i in range(19):
            h = heatmap_np[:, :, i:i+1]
            cv2.imwrite(f"heatmap_{i}.png", np.concatenate(
                [h]*3, axis=2)*oriImg_np)
        return
